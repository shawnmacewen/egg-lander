...
class EggLanderMissionScene extends Phaser.Scene {
  ...
  private prevDockingAssistReserveRatio: number | null = null
  private prevLandingAssistReserveRatio: number | null = null
  ...
  private resetMissionEntities() {
    ...
    this.prevDockingAssistReserveRatio = null
    this.prevLandingAssistReserveRatio = null
  }
  ...
  private getDockingAssistCue(level: LevelConfig, dist: number, speed: number, aligned: boolean) {
    ...
    const reserveRatio = this.getAssistReserveRatio(dockingRatios)
    ...
    const pulseReadout = this.getAssistPulseReadout(dockingWorstRatio, this.prevDockingAssistWorstRatio)
    ...
    const reserveTrendReadout = this.getAssistReserveTrendReadout(reserveRatio, this.prevDockingAssistReserveRatio)
    this.prevDockingAssistWorstRatio = dockingWorstRatio
    this.prevDockingAssistAvgRatio = dockingAvgRatio
    this.prevDockingAssistReserveRatio = reserveRatio

    return `Dock ${riskState} • ${windowReadout} • ${lineReadout} • ${controlReadout} • ${vectorReadout} • ${bufferReadout} • ${focusReadout} • ${watchReadout} • ${stackReadout} • ${shapeReadout} • ${spreadReadout} • ${loadReadout} • ${ceilingReadout} • ${reserveReadout} • ${reserveTrendReadout} • ${driftReadout} • ${biasReadout} • ${trendReadout} • ${deltaReadout} • ${stabilityReadout} • ${pulseReadout} • ${confidenceReadout} • ${commitReadout} • ${tempoReadout} • D ${Math.round(dist)}/${level.orbitalDockRadius} ${distState} • S ${Math.round(speed)}/${level.orbitalSafeSpeed} ${speedState} • ETA ${etaReadout} • A ${alignState} • Fix: ${correction}`
  }
  ...
  private getLandingAssistCue(level: LevelConfig) {
    ...
    const reserveRatio = this.getAssistReserveRatio(landingRatios)
    const biasReadout = this.getAssistBiasReadout([
    ...
    const pulseReadout = this.getAssistPulseReadout(landingWorstRatio, this.prevLandingAssistWorstRatio)
    ...
    const reserveTrendReadout = this.getAssistReserveTrendReadout(reserveRatio, this.prevLandingAssistReserveRatio)
    this.prevLandingAssistWorstRatio = landingWorstRatio
    this.prevLandingAssistAvgRatio = landingAvgRatio
    this.prevLandingAssistReserveRatio = reserveRatio

    return `Landing ${riskState} • ${windowReadout} • ${lineReadout} • ${controlReadout} • ${vectorReadout} • ${bufferReadout} • ${focusReadout} • ${watchReadout} • ${stackReadout} • ${shapeReadout} • ${spreadReadout} • ${loadReadout} • ${ceilingReadout} • ${reserveReadout} • ${reserveTrendReadout} • ${driftReadout} • ${biasReadout} • ${trendReadout} • ${deltaReadout} • ${stabilityReadout} • ${pulseReadout} • ${confidenceReadout} • ${commitReadout} • ${tempoReadout} • V ${Math.round(vertical)}/${Math.round(verticalLimit)} ${vState} • H ${Math.round(horizontal)}/${Math.round(horizontalLimit)} ${hState} • A ${Math.round(angleDeg)}°/${Math.round(angleLimitDeg)}° ${aState} • Alt ${altitude}px • ETA ${etaReadout} • Fix: ${correction}`
  }
  ...
  private getAssistWatchReadout(ratios: number[], labels: string[]) {
    if (ratios.length < 2) return 'Watch CLEAR'

    let worstIndex = 0
    let secondWorstIndex = 1
    if ((ratios[1] ?? 0) > (ratios[0] ?? 0)) {
      worstIndex = 1
      secondWorstIndex = 0
    }

    for (let i = 2; i < ratios.length; i += 1) {
      const ratio = ratios[i] ?? 0
      if (ratio > (ratios[worstIndex] ?? 0)) {
        secondWorstIndex = worstIndex
        worstIndex = i
      } else if (ratio > (ratios[secondWorstIndex] ?? 0)) {
        secondWorstIndex = i
      }
    }

    const worstRatio = ratios[worstIndex] ?? 0
    const secondWorstRatio = ratios[secondWorstIndex] ?? 0
    if (worstRatio - secondWorstRatio > 0.2 || secondWorstRatio <= 0.5) return 'Watch CLEAR'
  ...
  private getAssistShapeReadout(ratios: number[]) {
    if (ratios.length < 2) return 'Shape PINPOINT'

    const sorted = [...ratios].sort((a, b) => b - a)
    const worst = sorted[0] ?? 0
    const second = sorted[1] ?? 0

    if (worst < 0.6) return 'Shape EVEN'

    const gap = worst - second
  ...
  private getAssistReserveRatio(ratios: number[]) {
  ...
  private getAssistReserveReadout(ratios: number[]) {
    const reserveRatio = this.getAssistReserveRatio(ratios)
    if (reserveRatio === null) return 'Reserve --'

    const reservePercent = Math.max(0, Math.round((1 - reserveRatio) * 100))
    return `Reserve ${reservePercent}%`
  }

  private getAssistReserveTrendReadout(currentReserveRatio: number | null, prevReserveRatio: number | null) {
    if (currentReserveRatio === null || prevReserveRatio === null) return 'Reserve Drift HOLD'

    const delta = currentReserveRatio - prevReserveRatio
    if (delta <= -0.03) return 'Reserve Drift RISING'
    if (delta >= 0.03) return 'Reserve Drift FALLING'
    return 'Reserve Drift HOLD'
  }

  private getAssistSpreadReadout(ratios: number[]) {
    if (ratios.length === 0) return 'Spread 0%'

    let minRatio = ratios[0]
    let maxRatio = ratios[0]
    for (let i = 1; i < ratios.length; i += 1) {
      if (ratios[i] < minRatio) minRatio = ratios[i]
      if (ratios[i] > maxRatio) maxRatio = ratios[i]
    }

    const spreadPercent = Math.round(Math.max(0, maxRatio - minRatio) * 100)
  ...
  private getAssistTrendReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    if (prevWorstRatio === null) return 'Trend HOLD'
  ...
  private getAssistStabilityReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    if (prevWorstRatio === null) return 'Stability STEADY'

    const volatility = Math.abs(currentWorstRatio - prevWorstRatio)
    if (volatility >= 0.1) return 'Stability SWING'
    if (volatility >= 0.05) return 'Stability SHAKY'
    return 'Stability STEADY'
  }

  private getAssistPulseReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    const volatility = prevWorstRatio === null ? 0 : Math.abs(currentWorstRatio - prevWorstRatio)

    if (currentWorstRatio > 1.1) return 'Pulse SPIKE'
    if (currentWorstRatio > 1) return 'Pulse HOT'
    if (volatility >= 0.1) return 'Pulse SWING'
    if (currentWorstRatio <= 0.75) return 'Pulse CALM'
    return 'Pulse ACTIVE'
  }

  private getAssistConfidenceReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    const improving = prevWorstRatio !== null && currentWorstRatio <= prevWorstRatio - 0.03

    if (currentWorstRatio <= 0.75) {
      return improving ? 'Confidence HIGH+' : 'Confidence HIGH'
    }
    if (currentWorstRatio <= 1) {
      return improving ? 'Confidence BUILDING' : 'Confidence MED'
    }
    return improving ? 'Confidence RECOVERING' : 'Confidence LOW'
  }

  private getAssistCommitReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    const improving = prevWorstRatio !== null && currentWorstRatio <= prevWorstRatio - 0.03

    if (currentWorstRatio <= 0.75) return 'Commit GO'
    if (currentWorstRatio <= 1) return improving ? 'Commit BUILD' : 'Commit HOLD'
    return improving ? 'Commit RECOVER' : 'Commit ABORT'
  }

  private getAssistTempoReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    const volatility = prevWorstRatio === null ? 0 : Math.abs(currentWorstRatio - prevWorstRatio)

    if (currentWorstRatio > 1.1) return 'Tempo HARD BRAKE'
    if (currentWorstRatio > 1) return 'Tempo BRAKE'
  ...
  private getAssistLineReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    const improving = prevWorstRatio !== null && currentWorstRatio <= prevWorstRatio - 0.03
  ...
  private getAssistVectorReadout(currentWorstRatio: number, prevWorstRatio: number | null) {
    if (prevWorstRatio === null) return 'Vector SETUP'

    const delta = currentWorstRatio - prevWorstRatio
    if (delta <= -0.05) return 'Vector INBOUND'
    if (delta >= 0.05) return 'Vector OUTBOUND'
    return currentWorstRatio <= 1 ? 'Vector HOLD' : 'Vector RECOVER'
  }
  ...
  private getOnFootObjectiveCue(targetX: number, label: string) {
    ...
    const leftBound = view.left + offscreenPadding
    const rightBound = view.right - offscreenPadding
